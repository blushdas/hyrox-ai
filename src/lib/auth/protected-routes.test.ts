...
test.each(["/", "/sign-in", "/api/waitlist", "/api/auth/callback/google", "/_next/static/test.js", "/favicon.ico", "/logo.svg", "/dashboardish"])("leaves %s public", (path) => expect(isProtectedPath(path)).toBe(false));
